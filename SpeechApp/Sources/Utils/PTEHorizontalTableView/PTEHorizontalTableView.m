...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    if ([self.delegate respondsToSelector:@selector(tableView:widthForCellAtIndexPath:)])
    {
        float height = [self.delegate tableView:self widthForCellAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:0]];
        int page = (scrollView.contentOffset.y + (0.5f * height)) / height;
        
        if([self.delegate respondsToSelector:@selector(tableView:didScrollToPageIndex:)]) {
            [self.delegate tableView:self didScrollToPageIndex:page];
        }
    }
}
